...
export default class ProductComponent {
    ...
    init() {
        ...
        this.getProductByCategoryId(this.currentlySelectedCategoryId, this.page, this.pageSize);
        ...
        categoryService.getCategories()
            .then(response => {
                for (const category of response) {
                    selectList.append(HTMLHelpers.createOptionElement(category.name, category.id));
                }

                selectList.addEventListener('change', (e) => {
                    this.currentlySelectedCategoryId = e.target.value;
                    const categoryTitle = e.target[e.target.selectedIndex].getAttribute('name');
                    HTMLHelpers.changeTextInElement(document.getElementById('category-title'), categoryTitle);
                    this.getProductByCategoryId(this.currentlySelectedCategoryId, this.page, this.pageSize);
                });
            });
    }
    ...
    async getProductByCategoryId(categoryId, page, pageSize) {
        const { products, total } = await this.productService.getProductByCategoryId(categoryId, page, pageSize);
        console.log(products);
        this.totalPages = total;
        let html = '';
        for (const product of products) {
            html += HTMLHelpers.createCardElement(product.id, product.title, product.desc, product.price, product.image);
        }
    ...
    setPageSizeSelection() {
        ...
        for (const value of this.pageSizeValues) {
            selectList.appendChild(HTMLHelpers.createOptionElement(
                value, value, value === this.defaultPageSize));
        }
    ...
    addPreviousNextButtonEventListeners() {
        ...
        document.getElementById('next').disabled = this.hasNextPage(this.totalPages, this.page, this.pageSize);
